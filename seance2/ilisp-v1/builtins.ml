...
let eval_add arg1 arg2 =
  match arg1, arg2 with
    |Atom u, Atom v ->
      let p = int_of_string u in
      let q = int_of_string v in
      Atom (string_of_int (p+q))
    |_ -> failwith "pas des entiers"
...
let eval_comp arg1 arg2 =
  match arg1, arg2 with
    |Atom u, Atom v ->
      let p = int_of_string u in
      let q = int_of_string v in
      if p < q then (Atom ("lower"))
      else if p = q then (Atom ("equal"))
      else (Atom ("higher"))
    |_ -> failwith "pas des entiers"

let eval_lower arg1 arg2 =
  match arg1, arg2 with
    |Atom u, Atom v ->
      let p = int_of_string u in
      let q = int_of_string v in
...
let eval_addlist arg =
  let rec add_aux arg =
    ...
    |Cons (x, xs) ->
      match x with
      |Atom (y) ->
        let n = int_of_string y in
        n + add_aux xs
      |_ -> failwith "mauvais type"
  in
  Atom (string_of_int (add_aux arg))
...
let all_builtins =
  [ (* Special forms *)
    ...
    "comp", SpecialForm2 eval_comp;
    "lower", SpecialForm2 eval_lower;
    "eq", SpecialForm2 eval_eq;
    "higher", SpecialForm2 eval_higher;
    "addlist", SpecialForm1 eval_addlist;
